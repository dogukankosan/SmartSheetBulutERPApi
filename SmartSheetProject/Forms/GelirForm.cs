...
{
    public partial class GelirForm : XtraForm
    {
        ...
        private async void GelirForm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    await TextLog.LogToSQLiteAsync("❌ GelirForm - Bulut ERP ayarları bulunamadı");
                    XtraMessageBox.Show(
                ...
                {
                    await TextLog.LogToSQLiteAsync("❌ GelirForm - SmartSheet API Token kayıtlı değil");
                    XtraMessageBox.Show(
            ...
            {
                await TextLog.LogToSQLiteAsync($"❌ GelirForm_Load hatası: {ex.Message}");
        ...
        private async Task LoadDataAsync()
        {
            try
            {
                ...
                {
                    await TextLog.LogToSQLiteAsync($"❌ GelirForm - Token alınamadı: {tokenResult.ErrorMessage}");
                ...
                if (!result.Success)
                {
                    await TextLog.LogToSQLiteAsync($"❌ GelirForm - BulutERP veri çekme hatası: {result.ErrorMessage}");
            ...
            {
                await TextLog.LogToSQLiteAsync($"❌ GelirForm LoadDataAsync hatası: {ex.Message}");
        ...
        private async Task LoadSheetFaturaKeysAsync()
        {
            ...
            {
                await TextLog.LogToSQLiteAsync($"❌ GelirForm LoadSheetFaturaKeysAsync hatası: {ex.Message}");
            }
        }
        private void GridView1_RowStyle(object sender, RowStyleEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                var fatura = gridView1.GetRow(e.RowHandle) as GelirFaturaModel;
                if (fatura != null)
                {
        ...
        private List<GelirFaturaModel> ConvertToModel(List<Dictionary<string, object>> data)
        {
            List<GelirFaturaModel> list = new List<GelirFaturaModel>();
            foreach (var dict in data)
            {
                try
                {
                    GelirFaturaModel model = new GelirFaturaModel
                    {
                ...
                {
                    TextLog.LogToSQLiteAsync($"❌ GelirForm ConvertToModel hatası: {ex.Message}").Wait();
                }
            }
            return list;
        }
        ...
        private async void BtnExcel_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                SaveFileDialog saveDialog = new SaveFileDialog
                {
                    ...
                    FileName = $"Gelir_Faturalari_{DateTime.Now:yyyyMMdd_HHmmss}.xlsx"
                };
            ...
            {
                await TextLog.LogToSQLiteAsync($"❌ GelirForm Excel aktarma hatası: {ex.Message}");
        ...
        private async void btn_Aktar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                List<GelirFaturaModel> secilifaturalar = new List<GelirFaturaModel>();
                List<string> varOlanFaturalar = new List<string>();
                foreach (int rowHandle in selectedRows)
                {
                    if (rowHandle < 0) continue;
                   GelirFaturaModel fatura = gridView1.GetRow(rowHandle) as GelirFaturaModel;
                    if (fatura != null)
                    {
                        string key = CreateFaturaKey(fatura.CARI_KODU, fatura.FATURA_NO);
                        if (sheetFaturaKeys.Contains(key))
                ...
                string onayMesaji = $"{secilifaturalar.Count} adet yeni fatura GELİR SmartSheet'e aktarılacak.";
                ...
                if (onay != DialogResult.Yes)
                    return;
                this.Cursor = Cursors.WaitCursor;
                var result = await SmartsheetService.AddMultipleGelirFaturaAsync(secilifaturalar);
                if (result.Success)
                {
                    await LoadSheetFaturaKeysAsync();
                    string basariMesaji = $"Aktarım başarılı!\n✅ {result.Count} fatura GELİR sheet'ine eklendi";
                    if (varOlanFaturalar.Count > 0)
                ...
                {
                    await TextLog.LogToSQLiteAsync($"❌ GelirForm aktarım hatası: {result.ErrorMessage}");
            ...
            {
                await TextLog.LogToSQLiteAsync($"❌ GelirForm btn_Aktar_Click hatası: {ex.Message}");
                XtraMessageBox.Show($"Aktarım hatası:\n{ex.Message}", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
